...
class Products extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      productDescriptionFull: [
        { title: "", description: "", descriptionImage: "" },
      ],
      productDescriptionFullEdit: [
        { title: "", description: "", descriptionImage: "" },
      ],
      images: [{ productImage: "" }],
      pictures: [],
      ...
      productSpecificationWeight : [],
      productSpecificationSize: [],
      productSpecificationSizeNumberOrRomanNumber: [],
    };
  ...
  componentDidMount() {
    ...
    fetch(base + "/apiv2/product_specification_names", {
      ...
      .then((specificationName) => {
        ...
        console.log(
          "Specification Data ...... : ",
          this.state.productsSpecificationName
        );
        return false;
      });
    ...
    fetch(base + "/api/product_specification_details_for_product_entry", {
      ...
      .then((specificationName) => {
        ...
        console.log(
          ...
          this.state.productsSpecificationDetails
        );
        return false;
      });
  ...
  editItem(event) {
    ...
    console.log(
      "Edit Item function called !",
      event.currentTarget.dataset["id"]
    );
    this.state.getEditId = event.currentTarget.dataset["id"];

    fetch(
      base + `/api/get-product-edit/?id=${event.currentTarget.dataset["id"]}`,
      {
        method: "GET",
        headers: { Authorization: "Atiq " + cookie.load("token") },
      }
      ...
      .then((products) => {
        ...
        this.setState({
          ...
          productSpecificationBoxFun1:
            products.data.productSpecificationBoxFun1,
  ...
  editDescription() {
    ...
    console.log(
      "this.state.counLooperForDescription : ",
      this.state.counLooperForDescription
    );
    ...
    console.log(
      "this.state.productDescriptionFullEdit : ",
      this.state.productDescriptionFullEdit
    );
          ...
          <div key={i}>
            <FormGroup row>
              ...
              <Col xs="12" md="9">
                <Row>
                  <Col md="11">
                    <Input
                      ...
                      value={this.state.description[j].title}
                    />
                  </Col>
                  <Col md="1">
                    <span>
                ...
                <Row>
                  <Col md="11">
                    <Input
                      ...
                      onChange={this.handleChange.bind(this, i)}
                      value={this.state.description[j].description}
                    />
                  </Col>
                  <Col md="1"></Col>
                </Row>
                ...
                <Row>
                  <Col md="12" md="9">
                    {this.state.descriptionPreviewImages[i] ? (
                      <img
                        src={this.state.descriptionPreviewImages[i]}
                        height="100"
                        width="100"
                      ></img>
                    ) : (
    ...
    console.log(
      "this.state.productDescriptionFullEdit for edit : ",
      this.state.productDescriptionFullEdit
    );
  }
  ...
  editSpecificationDetails() {
    ...
    for (let i = 0; i < this.state.productsSpecificationDetails.length; i++) {
      let parsed_specification_details_name = JSON.parse(
        this.state.productsSpecificationDetails[i].specification_details_name
      );
      ...
      for (let j = 0; j < parsed_specification_details_name.length; j++) {
        ...
        for (var x = 0; x < this.state.pSD.length; x++) {
          if (
            this.state.pSD[x].specificationDetailsName ==
            parsed_specification_details_name[j]
          ) {
            if (
              this.state.productsSpecificationDetails[i].category_id ==
                this.state.cId &&
              this.state.user_type ==
                this.state.productsSpecificationDetails[i].entry_user_type
            ) {
              this.setState({
              ...
              this.specficationsAll.push(
                <Col md="3">
                  {counters == 0 ? (
                    <Label htmlFor="productSpecificationDetails">
              ...
              this.specficationsAll.push(
                <Col md="9">
                  ...
                  <Input
                    type="text"
                    id="detailsId"
                    onChange={this.handleChangeSpecification.bind(
                      this,
                      parsed_specification_details_name[j]
                    )}
                    name={"detailsId"}
                    value={this.state.detailsId}
                  />
                </Col>
              );
  ...
  editSpecificationName() {
    ...
    for (var i = 0; i < this.state.aPSN.length; i++) {
      if (this.state.aPSN[i].type == 0) {
        this.state.editSpecificationNameArray.push(
          <div>
            {" "}
            <strong> {"Color (50*40 Image)"} : </strong>{" "}
          </div>
        );
                ...
                <tr>
                  ...
                  <td>
                    <input
                      ...
                      dataId={value.id}
                      onChange={(e) =>
                        this.onChangeImageHandler(e, value.id, "colorsImage")
                      }
                      type="file"
                      name="image"
                    />
                  ...
                  {value.isChecked == true ? (
                    <td>
                      <Input
                        ...
                        name="sizeValue"
                        value={value.id}
                        onClick={this.specificationBoxFun.bind(this)}
                      />{" "}
                      <span style={{ marginLeft: "2rem" }}> {value.size} </span>
                    </td>
                  ) : (
                    <td>
                      <Input
  ...
  onChangeHandler = (event, id, name) => {
    ...
    console.log(
      ...
      event.currentTarget.getAttribute("dataId")
    );
    console.log("onChangeHandler", name);
    ...
    if (name == "productImages") {
      ...
      fileNameExclude = fileNameExclude.substr(
        0,
        fileNameExclude.lastIndexOf(".")
      );
        ...
        setTimeout(() => {
          ...
          if (this.state.isSubmitAllowed == true) {
            ...
            reader.onloadend = () => {
              ...
              if (serialNumber == 3) {
                this.setState({ imagePreview3: reader.result });
              }
              if (serialNumber == 4) {
                this.setState({ imagePreview4: reader.result });
              }
              if (serialNumber == 5) {
                this.setState({ imagePreview5: reader.result });
              }
              if (serialNumber == 6) {
                this.setState({ imagePreview6: reader.result });
              }
              if (serialNumber == 7) {
                this.setState({ imagePreview7: reader.result });
              }
              if (serialNumber == 8) {
                this.setState({ imagePreview8: reader.result });
              }
              if (serialNumber == 9) {
                this.setState({ imagePreview9: reader.result });
              }
              if (serialNumber == 10) {
                this.setState({ imagePreview10: reader.result });
              }
              if (serialNumber == 11) {
                this.setState({ imagePreview11: reader.result });
              }
              if (serialNumber == 12) {
                this.setState({ imagePreview12: reader.result });
              }
              if (serialNumber == 13) {
                this.setState({ imagePreview13: reader.result });
              }
  ...
  handleSubmitUpdate(event) {
    ...
    console.log(
      "productSpecificationBoxFun1 (color check box value) : ",
      this.state.productSpecificationBoxFun1
    );
    console.log(
      "this.state.productSpecificationBoxFun size value) : ",
      this.state.productSpecificationBoxFun
    );
    ...
    data.append(
      "productImagesJson",
      JSON.stringify(this.state.productImagesJson)
    );
    data.append(
      "productSpecificationBoxFun",
      JSON.stringify(this.state.productSpecificationBoxFun)
    );
    data.append(
      "specificationDetailsFullState",
      JSON.stringify(this.state.specificationDetailsFullState)
    );
    data.append(
      "productDescriptionFull",
      JSON.stringify(this.state.productDescriptionFull)
    );
    ...
    data.append(
      "colorImageObjects",
      JSON.stringify(this.state.colorImageObjects)
    );
    data.append(
      "productSpecificationBoxFun1",
      JSON.stringify(this.state.productSpecificationBoxFun1)
    );
  ...
  handleSubmit(event) {
    ...
    if(this.state.Weight) this.state.productSpecificationWeight.push({'Weight'  : this.state.Weight});

    // if(this.state.Size) this.state.productSpecificationBoxFun.push({'Size':this.state.Size});

    if(this.state.Number)       this.state.productSpecificationSize.push({'Number'        : this.state.productSpecificationSizeNumberOrRomanNumber});
    if(this.state.Roman_Number) this.state.productSpecificationSize.push({'Roman_Number'  : this.state.productSpecificationSizeNumberOrRomanNumber});
    if(this.state.Height)       this.state.productSpecificationSize.push({'Height'        : this.state.Height});
    if(this.state.Length)       this.state.productSpecificationSize.push({'Length'        : this.state.Length});
    if(this.state.Width)        this.state.productSpecificationSize.push({'Width'         : this.state.Width});

    // this.state.productSpecificationBoxFun.push({'Size' : sizeSpec});

    // console.log(this.state.productSpecificationBoxFun);
    // return;
    ...
    console.log(
      "PRoduct images in base 64 : ",
      this.state.productResizedImages
    );

    if (
      this.state.colorImageObjects.length > 0 &&
      this.state.productSpecificationBoxFun1.length > 0 &&
      this.state.colorImageObjects.length !=
        this.state.productSpecificationBoxFun1.length
    ) {
      ...
      data.append(
        "productSpecificationWeight",
        JSON.stringify(this.state.productSpecificationWeight)
      );
      data.append(
        "productSpecificationSize",
        JSON.stringify(this.state.productSpecificationSize)
      );
      data.append(
        "productImagesJson",
        JSON.stringify(this.state.productImagesJson)
      ...
      console.log(
        ...
        this.state.categoryIdValue != "" ? true : false
      );
      ...
      if (
        ...
        this.state.productBrand == undefined
      ) {
        this.setState({
          brandRequired: true,
        });
      ...
      if (event.keyCode == 13) {
        ...
        setTimeout(() => {
          if (
            this.state.nameRequired == false &&
            this.state.brandRequired == false &&
            this.state.categoryRequired == false &&
            this.state.priceRequired == false &&
            this.state.vendorRequired == false &&
    ...
    if (this.state.categoryIdAtUpdateClicked != 0) {
      ...
      setTimeout(() => {
        this.setState((prevState) => ({
          productDescriptionFull: [
            ...prevState.productDescriptionFull,
            { title: "", description: "", descriptionImage: "" },
          ],
        }));
      }, 50);
  ...
  addClickForEdit() {
    ...
    this.setState((prevState) => ({
      productDescriptionFullEdit: [
        ...prevState.productDescriptionFullEdit,
        { title: "", description: "", descriptionImage: "" },
      ],
    }));

    console.log(
      "productDescriptionFull For Edit after addClickForEdit :",
      this.state.productDescriptionFullEdit
    );
  }
    ...
    return this.state.productSPDFull.map((el, i) => (
      <div key={i}>
        <FormGroup row>
          <Col xs="12" md="12">
            <Input
              placeholder="Destails Specification"
              name="spd"
              id="SPD"
              onChange={this.handleSPDFullChange.bind(this, i)}
            />
            <br />
          </Col>
        </FormGroup>
  ...
  handleChange(i, e) {
    ...
    if (name == "descriptionImage") {
      ...
      reader.onloadend = () => {
        ...
        this.setState({
          descriptionPreviewImages: [
            ...this.state.descriptionPreviewImages,
            reader.result,
          ],
        });
      };
        ...
        <Col xs="12" md="3">
          <label htmlFor="file-input">
            {i == 3 && imagePreview3 ? (
              ...
              <img width="120" height="100" src={imagePreview4} />
            ) : i == 5 && imagePreview5 ? (
              <img width="120" height="100" src={imagePreview5} />
            ) : i == 6 && imagePreview6 ? (
              <img width="120" height="100" src={imagePreview6} />
            ) : i == 7 && imagePreview7 ? (
              <img width="120" height="100" src={imagePreview7} />
            ) : i == 8 && imagePreview8 ? (
              <img width="120" height="100" src={imagePreview8} />
            ) : i == 9 && imagePreview9 ? (
              <img width="120" height="100" src={imagePreview9} />
            ) : i == 10 && imagePreview10 ? (
              <img width="120" height="100" src={imagePreview10} />
            ) : i == 11 && imagePreview11 ? (
              <img width="120" height="100" src={imagePreview11} />
            ) : i == 12 && imagePreview12 ? (
              <img width="120" height="100" src={imagePreview12} />
            ) : i == 13 && imagePreview13 ? (
              <img width="120" height="100" src={imagePreview13} />
            ) : (
              <img
                width="120"
                height="100"
                src={publicUrl + "/productFormatedImages/Asset" + i + ".png"}
              />
            )}
          </label>
          <input
            ...
            dataId={i}
            type="file"
            onChange={(e) => this.onChangeHandler(e, i, "productImages")}
            name="image"
          />
        </Col>
      );
    ...
    for (let i = 3; i < 13; i++) {
      ...
      for (let j = 0; j < this.state.images.length; j++) {
        ...
        if (this.state.images[j].serialNumber == i && countImageLoop == 0) {
          ...
          countImageLoop == 0 &&
          this.state.images.length == countImageSecondLoop
        ) {
          imageView.push(
            <Col xs="12" md="3">
              <label htmlFor="file-input">
                {i == 3 && imagePreview3 ? (
                  ...
                  <img
                    ...
                    height="100"
                    src={
                      publicUrl + "/productFormatedImages/Asset" + i + ".png"
                    }
                  />
                )}
              </label>
      ...
      <React.Fragment>
        <FormGroup row>
          <Col md="12">
            ...
            <br />
            <Label htmlFor="productImageNote">
              {" "}
              <strong>Note : </strong>Image Size : 2240*1680/1280*960/560*420 &
              Image Formate : jpg/jpeg/png/gif{" "}
            </Label>
            {this.state.imageRequired == true ? (
              <p style={{ color: "red" }}>*At least 1 image is required</p>
            ) : null}
          </Col>
        </FormGroup>
        <FormGroup row>{this.createImages()}</FormGroup>
      </React.Fragment>
    );
  }
  ...
  createUpdatedImageUI() {
    return (
      <React.Fragment>
        ...
        <FormGroup row>{this.createUpdatedImages()}</FormGroup>
      </React.Fragment>
    );
  }
  ...
  changeSpecification = (event) => {
    ...
    if (this.state.categoryIdAtUpdateClicked != 0) {
      ...
      this.state.productSKUcode =
        "BNJ-000" + this.state.vendorId + "-0000" + this.state.newProductCode;
      ...
      let counters = 0;
      console.log(
        "Consoling specifiation",
        this.state.productsSpecificationName
      );

      {
        this.state.productsSpecificationName.map(
          (productsSpecificationNameValue, key) =>
            event.target.value == productsSpecificationNameValue.category_id ? (
              <div row>
                {this.specficationsAll.push(
                  <Col md="3">
                    {counter == 0 ? "Product Specifications " : null}
                  </Col>
                )}
                {this.specficationsAll.push(
                  <Col md="9">
      ...
      console.log("Category Id value changed and now : ", event.target.value);
      {
        this.state.productsSpecificationDetails.map(
          (productsSpecificationDetailsValue, key) =>
            JSON.parse(
              productsSpecificationDetailsValue.specification_details_name
            ).map((productsSpecificationDetailsValueParsed, key) => (
              <div row>
                {event.target.value ==
                  productsSpecificationDetailsValue.category_id &&
                this.state.user_type ==
                  productsSpecificationDetailsValue.entry_user_type ? (
                    ...
                    {this.specficationsAll.push(
                      <Col md="9">
                        ...
                        <Input
                          ...
                          name={
                            "detailsId"
                          } /*value={productsSpecificationDetailsValueParsed+" : "}*/
                        />
                      </Col>
                    )}
                    {++counters}
      ...
      if (this.specficationsAll.length == 1) {
        this.specficationsAll.push(
          <div row>
            <Col md="12">
              <strong>Note : </strong>No Specificaton Found For This Product
              Category
            </Col>
          </div>
        );
    ...
    if (this.state.categoryIdAtUpdateClicked == 0) {
      ...
      if (this.state.isColorOrSizeClicked == true) {
...

        {
          this.state.productsSpecificationName.map(
            (productsSpecificationNameValue, key) =>
              ...
              productsSpecificationNameValue.category_id ? (
                <div row>
                  {this.specficationsAll.push(
                    <Col md="3">
                      {counter == 0 ? "Product Specifications " : null}
                        ...
                        <div>
                          ...
                          ))}
                          {
                            <Button
                              ...
                              onClick={this.addMoreColor.bind(this)}
                            >
                              {" "}
                              <i className="fa fa-plus-circle"></i> New Color
                            </Button>
                          }
                        </div>
                      ) : (
        ...
        {
          this.state.productsSpecificationName.map((productsSpecificationNameValue, key) => event.target.value == productsSpecificationNameValue.category_id ? (
                <div row>
                  {this.specficationsAll.push(
                    <Col md="3">
                      {counter == 0 ? "Product Specifications " : null}
    ...
    console.log(
      "this.state.categoryIdAtUpdateClicked : ",
      this.state.categoryIdAtUpdateClicked
    );
        ...
        (productsSpecificationDetailsValue, key) =>
          JSON.parse(
            ...
            event.target.value == productsSpecificationDetailsValueParsed
              ? this.displaySpecificationDetailsData.push(
                  <div>
                    <Input
                      type="text"
                      id="detailsId"
  ...
  toggleSmall(event) {
    ...
    if (event == "deleteSpecificationDetailsPermitted") {
      ...
      fetch(
        base +
          `/api/authorizeProductInfo/?id=${this.state.productIdForAuthorize}&clicked=${this.state.isAuthorizeClicked}`,
        {
          method: "GET",
          headers: new Headers({
            Authorization: "Atiq " + cookie.load("token"),
                    ...
                    <tr>
                      ...
                      <td>
                        {productListValue.home_image ? (
                          ...
                          <img
                            width="100"
                            height="80"
                            src={publicUrl + "/upload/vendor/default.png"}
                          ></img>
                        )}
                      </td>
                      ...
                      <td>
                        {this.state.productsCategory.map(
                          (productsCategoryValue, key) =>
                            productListValue.category_id ==
                            productsCategoryValue.id
                      ...
                      <td>
                        {this.state.vendorList.map((vendorListValue, key) =>
                          productListValue.vendor_id == vendorListValue.id
          ...
          <i className="fa fa-edit fa-lg"  title="Edit Products Info" aria-hidden="true" style={{color: '#009345'}}></i>
                          ...
                          {localStorage.user_type == "admin" ? (
                            <a
                              ...
                              ref="dataIds"
                              data-id={productListValue.id}
                              value={productListValue.id}
                            >
                              <i
                                className="fa fa-trash fa-lg"
                                title="Delete Products Details"
                                aria-hidden="true"
                                style={{ color: "#EB1C22" }}
                              ></i>
                            </a>
                        ...
                        <td>
                          {productListValue.isApprove == "authorize" ? (
                            <center>
                              ...
                              {/* <i className="fa fa-angle-left fa-lg" style={{color: '#EB1C22', marginTop: '50%', marginBottom: '50%'}}></i> */}
                            </center>
                          ) : (
                            <center>
                              <a
                                data-id={productListValue.id}
                                data-clicked={"Authorize"}
                                style={{ cursor: "pointer" }}
                                title="Authorize"
                                onClick={this.authorize.bind(this)}
                              >
        ...
        >
          <ModalHeader toggle={this.toggleSmall}>
            {this.state.isAuthorizeClicked == "Authorize" ? (
              <center>Product Authorization</center>
            ) : this.state.isAuthorizeClicked == "Unauthorize" ? (
              <center>Product Unauthorization</center>
            ) : (
          ...
          <ModalBody>
            <strong>
              {this.state.isAuthorizeClicked == "Authorize" ? (
                <center>Are Sure to Authorize this product ?</center>
              ) : this.state.isAuthorizeClicked == "Unauthorize" ? (
                <center>Are Sure to Unauthorize this product ?</center>
              ) : (
                <center>Are Sure to delete this product ?</center>
              )}
            </strong>
          </ModalBody>
          <ModalFooter>
            {this.state.isAuthorizeClicked == "Authorize" ? (
              <div>
                <Button
                  color="success"
                  onClick={(e) => {
                    this.toggleSmall("authorizeProductPermitted");
                  }}
                >
                  yes
                </Button>{" "}
                <Button
                  color="danger"
                  onClick={(e) => {
                    this.toggleSmall("authorizeProductDenied");
                  }}
                >
                  No
                </Button>
              ...
              <div>
                <Button
                  color="success"
                  onClick={(e) => {
                    this.toggleSmall("deleteSpecificationDetailsPermitted");
                  }}
                >
                  yes
                </Button>{" "}
                <Button
                  color="danger"
                  onClick={(e) => {
                    this.toggleSmall("deleteSpecificationDetailsDenied");
                  }}
                >
                  No
                </Button>
                      ...
                      <FormGroup row>
                        ...
                        <Col xs="12" md="9">
                          <Input
                            type="select"
                            name="cId"
                            onChange={this.changeSpecification}
                            id="cId"
                            value={this.state.cId}
                            disabled
                          >
                            <option value="0">Please select</option>
                            {this.state.productsCategoryList.map(
                              (productsCategoryListValue, key) => (
                                <option value={key}>
                                  {" "}
                                  {productsCategoryListValue}{" "}
                                </option>
                              )
                            )}
                            {/* {
    ...
    <option value={key}> {productsCategoryListValue} </option>
      ...
      null
                        ...
                        <Col md="9">
                          {" "}
                          {this.state.isCategoryChangedForUpdate == 0
                            ? this.state.editSpecificationNameArray
                            : null}{" "}
                        </Col>
                      </FormGroup>
  ...
  null
                      ...
                      <FormGroup row>
                        ...
                        <Col xs="12" md="9">
                          <TagInput
                            ...
                            placeholder="[Key Words] Type tags & hit enter.."
                    ...
                    >
                      ...
                      <FormGroup row>
                        ...
                        <Col xs="12" md="9">
                          <Button
                            color="primary"
                            className="btn-pill btn btn-light btn-block"
                            size="sm"
                            onClick={this.addClick.bind(this)}
                          >
                            {" "}
                            <i className="fa fa-plus-square"></i> Add More
                          </Button>
                          &nbsp;
                        </Col>
                      </FormGroup>
        ...
        >
          <ModalHeader toggle={this.toggleSuccessSize}>
            Add New Size
          </ModalHeader>
          <ModalBody>
            <Input
        ...
        <Modal
          ...
          className={"modal-success " + this.props.className}
        >
          <ModalHeader toggle={this.toggleSuccessColor}>
            Add New Color
          </ModalHeader>
          <ModalBody>
            <Input
        ...
        >
          <ModalHeader toggle={this.toggleImageAlert}>
            {" "}
            <strong>Alert !</strong>{" "}
          </ModalHeader>
          <ModalBody>
            <h4>
              {" "}
              <strong> Can't upload this type of image! </strong>{" "}
            </h4>
            <strong> Note : </strong> Image Size : 2240*1680/1280*960/560*420 &
            Image Formate : jpg/jpeg/png/gif
          </ModalBody>
        ...
        <Modal
          ...
          className={"modal-danger " + this.props.className}
        >
          <ModalHeader toggle={this.toggleColorImageAlert}>
            {" "}
            <strong>Alert !</strong>{" "}
          </ModalHeader>
          <ModalBody>
            ...
            <strong> Note : </strong> Image Size : 50*40 & Image Formate :
            jpg/jpeg/png
          </ModalBody>
        </Modal>
